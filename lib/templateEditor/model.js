...
module.exports = class Model {
  ...
  addItem({ type, parentViewItem }) {
    ...
    parentViewItem.expanded = true;
    this._afterUpdate();
  }

  deleteItem({ targetItem }) {
    const { parent, index } = this.getPosition(targetItem);

    parent.items.splice(index, 1);
    this._afterUpdate();
  }

  moveItem({ parent, targetItem, index }) {
    const positionTargetItem = this.getPosition(targetItem);

    positionTargetItem.parent.items.splice(positionTargetItem.index, 1);

    if (index != null) {
      if (positionTargetItem.parent === parent) {
        if (positionTargetItem.index > index) {
          index++;
        }
      } else {
        index++;
      }
      parent.items.splice(index, 0, targetItem);
    } else {
      parent.items.push(targetItem);
    }

    this._afterUpdate();
  }

  //TODO fix position Form
  getPosition(target) {
    let result;
    const findParent = viewItem => {
      if (viewItem.items && viewItem.items.length) {
        viewItem.items.forEach((item, index) => {
          if (item === target) {
            result = {
              parent: viewItem,
              index
