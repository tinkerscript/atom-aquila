...
class Element {
  ...
  _renderTree() {
    ...
    viewModel.forEach(({ id, level, isSelected, item }) => {
      ...
      betweenDiv.addEventListener('dragover', () => {
        betweenDiv.classList.add('atom-aquila-tree-between-over');
      });
      betweenDiv.addEventListener('dragleave', () => {
        betweenDiv.classList.remove('atom-aquila-tree-between-over');
      });
      ...
      itemDiv.addEventListener('drop', () => {
        itemDiv.classList.remove('atom-aquila-tree-item_selected');
        this._model.moveItem({ parent: item, targetItem: dragedItem });
        this._renderTree();
      });

      betweenDiv.addEventListener('drop', () => {
        betweenDiv.classList.add('atom-aquila-tree-between-over');
        
        let { index, parent } = this._model.getPosition(item);
        this._model.moveItem({ parent, index, targetItem: dragedItem });
      ...
      betweenDiv.classList.add('atom-aquila-tree-between');
      betweenDiv.style['margin-left'] = `${leftMargin}em`;

      itemDiv.classList.add('atom-aquila-tree-item');
      itemDiv.classList.add(`aquila-control-${type}`);
      ...
      this._tree.appendChild(betweenDiv);
    });
