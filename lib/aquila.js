...
module.exports = {
  activate() {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace',{
      ...
      'aquila:status': () => {
        let message = `${name}@${version}: загружен.<br/>`;
        if (Object.keys(manifestHelper.manifests).length) {
          message += 'Версии манифестов контролов: <br/>';
          message = Object.keys(manifestHelper.manifests).reduce((result, path, index) => {
            return result + `${index+1}. ${path} - ${manifestHelper.manifests[path].version}<br/>`
          }, message);
        }
